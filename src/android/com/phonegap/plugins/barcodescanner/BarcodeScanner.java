...
public class BarcodeScanner extends CordovaPlugin {
  ...
  public void scan(final JSONArray args) {
    ...
    cordova.getThreadPool().execute(new Runnable() {
      public void run() {
          ...
          for (int i = 0; i < args.length(); i++) {
            ...
            for (int j = 0; j < names.length(); j++) {
              try {
                ...
                Log.i("Billy - key ", key);
                Log.i("Billy - value",value.toString());

                if(key.equals("workSpaceList")) {

                  JSONArray jsonArray = new JSONArray(obj.get(key).toString());

                  String[] workspace = new String[jsonArray.length()];

                  for (int k = 0; k < jsonArray.length(); k++) {
                    workspace[k] = jsonArray.getString(k);

                    Log.i("Billy - " + k, jsonArray.getString(k));
                  }

                  Log.i("Billy - work", workspace.toString());

                  intentScan.putExtra("WORKSPACE_VALUE", workspace);
        ...
        //String[] sampleValue = new String[3];
        //sampleValue[0] = "15.40c";
        //sampleValue[1] = "15.50c";
        //sampleValue[2] = "15.60c";

        //intentScan.putExtra("WORKSPACE_VALUE", sampleValue);
  ...
  private String makeOneString(String[] strArray){
    String retVal = "";
    ...
    if (requestCode == REQUEST_CODE && this.callbackContext != null) {
      if (resultCode == Activity.RESULT_OK) {
        ...
        try {
          ...
          String[] resultList = intent.getStringArrayExtra("SCAN_RESULT_LIST");

          //String actions =  intent.getAction();
          Log.i("Billy - ", resultList[0]);

          if( resultList != null){

            obj.put(TEXT, makeOneString(resultList));
            Log.i("Billy - ", resultList[0]);
          }
          else {
            Log.i("Billy", "no value on the list");
          }
          //obj.put(TEXT, intent.getStringArrayExtra("SCAN_RESULT_LIST"));
        ...
        //Log.i("Hoi Mao ni sya", intent.getStringArrayExtra("SCAN_RESULT_LIST").toString());
